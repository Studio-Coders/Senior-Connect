...
// -------------------------------------------
    ...
    /*for (var i = 0; i < finals.length; i++) {
        ...
        for (var j = 0; j < finals[j].matches.length; j++) {
            var seniorIntEx = getIntExByName(seniors, finals[i].matches[j].name);
            var diff = Math.abs(volunteerIntEx - seniorIntEx);
...
// -------------------------------------------
    ...
    for (var i = 0; i < finals.length; i++) {
        ...
        */
        ...
        // Array of the Senior Names who have the lowest visit numbers
        var minimumSeniorsArr = [];
        var minimumDiff = 1000;
        var volunteerIntEx = getIntExByName(volunteers, volunteerTemplate.name);
        // Iterate through the seniors for that volunteer
        for (var j = 0; j < seniorTemplate.length; j++) {
            var seniorName = seniorTemplate[j].name;
            var seniorIntEx = getIntExByName(seniors, seniorName);
            var diff = Math.abs(volunteerIntEx - seniorIntEx);
            if (diff < minimumDiff) {
                minimumDiff = diff;
                minimumSeniorsArr = [];
...
results = matchByNumVisits(results, seniors);
results = matchByPreference(results);
...
// for loop to print
    ...
    // For every senior in matches, find their index score using their name
    // Create variable called smallest diff
    // Calculate the difference in index score between volunteer and senior
